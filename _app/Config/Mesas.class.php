...
class Mesas{
    ...
    private function CheckMesa(){
        $Read = new Read();
        $Read->ExeRead(self::Entity, "WHERE id_db_settings=:ip AND name=:n AND capacidade=:cap", "ip={$this->id_db_settings}&n={$this->Data['name']}&cap={$this->Data['capacidade']}");

        if($Read->getResult()):
            $this->Error  = ["Ops: a mesa <strong>{$this->Data['name']}</strong> já encontra-se registrada no Kwanzar!", WS_INFOR];
            $this->Result = false;
        else:
            $this->Result = true;
        endif;
    ...
    private function Create(){
        ...
        if($Create->getResult()):
            ...
            $this->Error  = ["Ops: aconteceu um erro inesperado ao cadastrar a mesa <strong>{$this->Data['name']}</strong>.!", WS_ALERT];
            $this->Result = false;
        endif;
    }
    ...
    public function ExeDelete($id, $id_db_kwanzar){
        ...
        if($Delete->getResult() || $Delete->getRowCount()):
            $this->Error  = ["Mesa detelada com sucesso!", WS_INFOR];
            $this->Result = true;
        else:
