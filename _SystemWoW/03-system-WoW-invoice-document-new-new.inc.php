...
if($read->getResult()):
    ...
    <div class="header">
        ...
        <div class="limpopo-silvio">
            <h1><?php if($k['InvoiceType'] == 'GT'): echo "Guia de transporte ";  endif; ?> <?= $k['InvoiceType']." ".$k['mes'].$k['Code'].$k['ano']."/".$k['numero']; if(Strong::Config($id_db_settings)['JanuarioSakalumbu'] == 2 || DBKwanzar::CheckConfig($id_db_settings)['JanuarioSakalumbu'] == '' || Strong::Config($id_db_settings)['JanuarioSakalumbu'] == null): echo '&nbsp;MODO DE INSTRUÇÃO, ESSE DOCUMENTO NÃO É VÁLIDO'; endif; ?></h1>
        ...
        <table class="">
            ...
            <tbody>
            <tr>
                <td><?= $k['guid_name']."; ".$k['guid_matricula']; ?></td>
                <td><?= $k['settings_endereco'] ?></td>
                <td><?= $k['guid_endereco']."; ".$k['guid_city']."; ".$k['guid_postal']; ?></td>
                <td><?= $k['guid_obs'] ?></td>
            </tr>
            </tbody>
        </table>
