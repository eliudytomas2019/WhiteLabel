...
if($read->getResult()):
    ...
    <div class="heliospro">
        <div id="xuxu">
            <h1 class="header-one pussy">Receita Médica</h1></div>
    </div>

    <div class="jugadoresI">
        <?php
        $Read->ExeRead("cv_clinic_receita", "WHERE id_paciente=:userid AND id_db_settings=:i AND id=:ui ", "userid={$id_paciente}&i={$id_db_settings}&ui={$postId}");
        ...
        if($Read->getResult()):
            foreach ($Read->getResult() as $key):
                ...
                <br/><strong>Observações: <p><?= $key['observacoes']; ?></p></strong>
                <?php
            endforeach;
        endif;
        ?>
